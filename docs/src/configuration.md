...
| `src_dirs` | array of strings | varies | Directories to scan for source files. **No processor scans the project tree by default** — an omitted or empty `src_dirs` (`[]`) matches no files at all, so a processor declared without it simply builds nothing. Some processors have a specific default (e.g., `tera` defaults to `"tera.templates"`, `cc_single_file` defaults to `"src"`); the rest default to `[]` and must be told where to look. To scan the whole project deliberately, use `src_dirs = [""]` — an empty string means the project root and walks everything beneath it, including `node_modules/`, `.venv/` and `target/` unless excluded. `src_files` is an alternative to `src_dirs` for listing exact paths. **Every entry must exist on disk** (or be the declared output target of an upstream processor) — a missing directory fails the build with `src_dirs entry 'X' does not exist or is not a directory`, unless `[build] skip_missing_src_dirs = true` is set, in which case the missing entry is skipped and the processor simply matches no files there. Use `rsconstruct processors defconfig <name>` to see a processor's defaults. |
